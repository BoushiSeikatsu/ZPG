...
*/
Triangle::Triangle(GLuint* VBO, GLuint* VAO, int startingPosition, int count, int Flags) : Shape(startingPosition, count)
{
    this->Flags = Flags;
    setupVertexObjects(VBO, VAO);
}
//Pro zatim 3 moznosti, upravim potom podle potreby
bool Triangle::setupVertexObjects(GLuint* VBO, GLuint* VAO)
{
    if (Flags == VERTICES)
    {
...
bool Triangle::createShape()
{
    glDrawArrays(GL_TRIANGLES, startingPosition, count);
    return true;
}

bool Triangle::deleteShape()
{
    return false;
}

bool Triangle::updateShape()
{
    return false;
}
